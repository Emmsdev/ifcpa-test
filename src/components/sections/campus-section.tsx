...
export function CampusSection({ locale }: { locale: Locale }) {
  ...
  return (
    <section id="campus" className="section-shell bg-[var(--surface-container-low)] py-24 lg:py-36">
      <div className="mx-auto grid max-w-[1380px] items-center gap-14 lg:grid-cols-12 lg:gap-20">
        <div className="lg:col-span-5">
          ...
          <div className="mt-7 flex flex-wrap gap-2">
            {(t.tags as string[]).map((tag) => <span key={tag} className="border-l-2 border-[var(--secondary)] bg-[var(--surface-bright)] px-3 py-2 text-xs font-semibold text-[var(--on-secondary-fixed-variant)]">{tag}</span>)}
          </div>
          <a href="https://www.facebook.com/ifcpacrtv/" target="_blank" rel="noreferrer" className="app-button group mt-9 inline-flex items-center gap-3 bg-[var(--primary)] px-5 py-3 text-sm font-semibold text-white">
            {t.action as string}<span aria-hidden="true" className="transition-transform group-hover:translate-x-1">↗</span>
          </a>
        </div>
        <div data-scroll-item className="grid min-h-[540px] grid-cols-2 grid-rows-2 gap-3 lg:col-span-7">
          <figure className="frame-corners group relative col-span-2 row-span-1 min-h-[320px] overflow-hidden bg-[var(--primary)] shadow-[var(--shadow-soft)] sm:row-span-2">
          ...
          <figure className="group relative min-h-[210px] overflow-hidden border-4 border-[var(--surface-container-low)] bg-[var(--primary)] shadow-[var(--shadow-soft)] sm:col-start-2 sm:row-start-1 sm:ml-12 sm:mt-8">
          ...
          <figure className="group relative min-h-[210px] overflow-hidden border-4 border-[var(--surface-container-low)] bg-[var(--primary)] shadow-[var(--shadow-soft)] sm:col-start-2 sm:row-start-2 sm:-ml-8 sm:mb-8">
